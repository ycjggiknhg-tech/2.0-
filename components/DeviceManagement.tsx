...
import { Search, Filter, Battery, MapPin, Shield, Zap, MoreHorizontal, Plus } from 'lucide-react';
    ...
    <div className="p-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
        <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm text-left">
          ...
          <p className="text-2xl font-black text-slate-900">{devices.length} <span className="text-xs text-green-500 font-bold ml-1">活跃</span></p>
        </div>
        <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm text-left">
          <p className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-1">待充电/换电</p>
          <p className="text-2xl font-black text-slate-900">{devices.filter(d => d.status === '低电量').length} <span className="text-xs text-orange-500 font-bold ml-1">需关注</span></p>
        </div>
        <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm text-left">

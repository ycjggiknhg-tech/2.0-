...
import { 
  ...
  UserPlus, Phone, AlertCircle, Users, CheckCircle2, XCircle, TrendingUp, Edit3, Check, Save
} from 'lucide-react';
...
const Recruitment: React.FC<RecruitmentProps> = ({ onAction, applicants, setApplicants, onOpenPublicForm, onHire }) => {
  ...
  const handleUpdateStation = (id: string) => {
    ...
    setApplicants(prev => prev.map(app => 
      app.id === id ? { 
        ...app, 
        station: tempStationName.trim(),
        assignmentStatus: `已分配至 ${tempStationName.trim()}`
      } : app
    ));
    
    setEditingStationId(null);
    onAction(`✅ 已将应聘者成功分配至：${tempStationName.trim()}`);
  };
    ...
    <div className="p-8 text-left animate-in fade-in duration-500">
      <div className="flex justify-between items-center mb-8">
        <div>
          ...
          <p className="text-slate-500 text-sm">实时监控各城市面试进度，手动分配入职站点。</p>
        </div>
          ...
          <table className="w-full min-w-[1100px] border-collapse table-auto">
            <thead>
              <tr className="text-left bg-slate-50 border-b border-slate-100">
                ...
                <th className="px-6 py-5 text-[10px] font-black uppercase text-slate-400 w-48">站点分配进度</th>
                <th className="px-6 py-5 text-[10px] font-black uppercase text-slate-400">所属站点 (点击分配)</th>
                <th className="px-6 py-5 text-[10px] font-black uppercase text-slate-400">联系电话</th>
                <th className="px-6 py-5 text-[10px] font-black uppercase text-slate-400 text-right">管理操作</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100 text-left">
              {applicants.map((applicant, index) => {
                const isAssigned = applicant.station !== '默认站点' && applicant.station !== '';
                const isEditingStation = editingStationId === applicant.id;
                ...
                return (
                  <tr key={applicant.id} className="hover:bg-slate-50/50 transition-colors group">
                    ...
                    <td className="px-4 py-5 whitespace-nowrap">
                      <div className="flex items-center gap-2">
                        <div className="w-8 h-8 rounded-xl bg-blue-600 flex items-center justify-center font-black text-white text-[10px] shadow-sm">{applicant.name.charAt(0)}</div>
                        <div>
                          <p className="font-bold text-slate-900 text-sm">{applicant.name}</p>
                          <p className="text-[9px] text-slate-400 font-bold uppercase font-mono">{applicant.appliedDate}</p>
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-5">
                      <div className="relative inline-block w-36">
                        ...
                        >
                          ...
                          <option value="failed">拒绝/淘汰</option>
                        </select>
                        <ChevronDown size={14} className="absolute right-2 top-1/2 -translate-y-1/2 pointer-events-none opacity-40" />
                      </div>
                    </td>
                    <td className="px-6 py-5">
                      <div className={`flex items-center gap-1.5 text-[10px] font-black uppercase px-3 py-1 rounded-lg border w-fit ${
                        isAssigned 
                          ? 'bg-emerald-50 text-emerald-600 border-emerald-100' 
                          : 'bg-slate-50 text-slate-400 border-slate-200'
                      }`}>
                        {isAssigned ? <CheckCircle2 size={12} /> : <AlertCircle size={12} />}
                        {isAssigned ? '已分配站点' : '待分配站点'}
                      </div>
                    </td>
                    <td className="px-6 py-5">
                      {isEditingStation ? (
                        <div className="flex items-center gap-1 animate-in slide-in-from-left-2 duration-200">
                          <input 
                            autoFocus
                            className="bg-white border-2 border-blue-500 px-3 py-1.5 rounded-xl text-xs font-bold outline-none shadow-lg shadow-blue-100 w-40"
                            value={tempStationName}
                            placeholder="输入站点名称"
                            onChange={(e) => setTempStationName(e.target.value)}
                            onKeyDown={(e) => e.key === 'Enter' && handleUpdateStation(applicant.id)}
                          />
                          <button 
                            onClick={() => handleUpdateStation(applicant.id)}
                            className="p-1.5 bg-blue-600 text-white rounded-lg hover:bg-blue-700 shadow-md"
                          >
                            <Check size={16} />
                          </button>
                          <button 
                        ...
                        <div 
                          onClick={() => {
                            setEditingStationId(applicant.id);
                            setTempStationName(applicant.station === '默认站点' ? '' : applicant.station);
                          }}
                          className="flex flex-col text-sm cursor-pointer hover:bg-blue-50/50 p-2 rounded-xl transition-all border border-transparent hover:border-blue-100 group/station"
                        >
                          <span className="font-bold text-slate-800 flex items-center gap-1">
                            {applicant.city} 
                            <Edit3 size={12} className="text-blue-400 opacity-0 group-hover/station:opacity-100 transition-opacity" />
                          </span>
                          <span className={`text-[10px] font-black uppercase tracking-tighter ${applicant.station === '默认站点' ? 'text-rose-400' : 'text-slate-400'}`}>
                            {applicant.station}
                          </span>
                        </div>
                      )}
                    ...
                    <td className="px-6 py-5 text-right relative">
                      <button 
                        ...
                        className={`px-3 py-1.5 rounded-lg border text-[10px] font-black uppercase transition-all ml-auto flex items-center gap-2 ${
                      ...
                      >
                        管理 <ChevronDown size={12} />
                      </button>
                      {activeMenuId === applicant.id && (
                        <div className="absolute right-6 mt-1 w-48 bg-white border border-slate-100 rounded-xl shadow-2xl z-50 p-1 space-y-0.5 animate-in fade-in zoom-in-95 duration-100 text-left">
                          <button 
                            onClick={(e) => { 
                              e.stopPropagation(); 
                              if (applicant.entryResult !== 'passed') {
                                onAction('⚠️ 请先将面试结论修改为“面试通过”再办理入职');
                                return;
                              }
                              if (!isAssigned) {
                                onAction('⚠️ 办理入职前，请先手动分配入职站点');
                                setEditingStationId(applicant.id);
                                return;
                              }
                              onHire(applicant); 
                            ...
                            className={`w-full flex items-center gap-2 px-3 py-2 text-xs font-bold rounded-lg ${
                              isAssigned && applicant.entryResult === 'passed' 
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in">
          <div className="bg-white rounded-[2.5rem] w-full max-w-sm shadow-2xl p-8 text-center animate-in zoom-in-95">
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-lg font-black uppercase tracking-tight">扫码录入应聘者</h2>
